...
R2.vals = array(data=NA,dim=c(7,1))
site.names = c("mpj","seg","ses","vcm","vcp","vcs","wjs")
rownames(R2.vals) = site.names
colnames(R2.vals) = "R2"
coef.vals = array(data=NA,dim=c(2*7,4))
rownames(coef.vals) = c("mpj","mpj","seg","seg","ses","ses","vcm","vcm","vcp","vcp",
                        "vcs","vcs","wjs","wjs")
...
ss = max((1:7)*match(site.names,site),na.rm=TRUE)
...
GelRaft = array(data=NA,dim=c(7,4))
